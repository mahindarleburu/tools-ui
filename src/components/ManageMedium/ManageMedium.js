...
import { listAllMedium, deleteMedium, updateMedium, createMedium, listMediumSelector } from "./ManageMediumSlice";
...
const ManageMediums = () => {
  ...
  const { isFetching, isSuccess, isError, errorMessage, listMedium } = useSelector(listMediumSelector);
  ...
  const listDataapi = async (uid) => {
    dispatch(listAllMedium({ user_id: googleUser.uid }));
  };
  useEffect(() => {
    listDataapi();
  }, []);
  ...
  const handleCreateSaveRow = async () => {
    ...
    if (errors.display_name === false) {
      dispatch(createMedium(selectedRow));
      handleCreateClose();
      setSelectedRow({});
    }
  };
  ...
  const handleEditSaveRow = async () => {
    //if using flat data and simple accessorKeys/ids, you can just do a simple assignment here.
    if (errors.display_name === false) {
      dispatch(updateMedium(selectedRow));
      handleEditClose();
      setSelectedRow({});
    }
  };
  ...
  const handleDelete = async () => {
    //if using flat data and simple accessorKeys/ids, you can just do a simple assignment here.
    if (errors.display_name === false) {
      dispatch(deleteMedium(selectedRow));
      handleDeleteClose();
      setSelectedRow({});
    }
  };
  return (
    <>
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            label="Display Name"
            helperText={
              errors.display_name ? "Display Name is Empty, Please enter the Display Name" : "Enter the Display Name"
            }
            type="text"
            fullWidth
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Delete Medium"}</DialogTitle>
        <DialogContent>
